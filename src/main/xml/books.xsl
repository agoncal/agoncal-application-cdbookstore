<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="Items">
        ...
        <xsl:for-each select="Item">INSERT INTO ITEM (id, version, discriminator, isbn, title, rank, small_image_url, medium_image_url, language, unit_cost, nb_of_pages, publication_date, category_id, publisher_id, description) VALUES ( <xsl:variable name="i" select="position()" /> <xsl:value-of select="$i"/> <xsl:text>, 1, 'B', </xsl:text>
            ...
            <xsl:if test="ItemAttributes/ListPrice/FormattedPrice"><xsl:value-of select="normalize-space(ItemAttributes/ListPrice/FormattedPrice)"/></xsl:if>
